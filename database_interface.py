...
# Dictionary wich maps the name of a defect to its corresponding ID in the database,
...
class DBClient:
    ...
    """
    Interface to insert info of the product. Note that the date must be in the format 
    ...
    def insertImage(self, productID, image_storage_path):
        access_point = '{base_service}/{end_point}'.format(
                base_service=URL_SERVICE,end_point=IMAGE_END_POINT)
        r = requests.get(access_point, headers=DEFAULT_HEADERS, params=PARAMS_MAX_ID)
        ID = self.getNextID(r.json())
    ...
    def insertProductsDefects(self, productID, defectID):
        access_point = '{base_service}/{end_point}'.format(
                base_service=URL_SERVICE,end_point=PRODUCT_DEFECT_END_POINT)
        r = requests.get(access_point, headers=DEFAULT_HEADERS, params=PARAMS_MAX_ID)
        ID = self.getNextID(r.json())
